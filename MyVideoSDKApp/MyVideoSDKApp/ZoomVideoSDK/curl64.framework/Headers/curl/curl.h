...
//@@zoom_changes_insert_point_begin
  ...
  CURLOPT(CURLOPT_ENABLE_EPA_OPENSSL, CURLOPTTYPE_LONG, 329),

  /* DNS resolution timeout in milliseconds */
  CURLOPT(CURLOPT_DNS_TIMEOUT_MS, CURLOPTTYPE_LONG, 330),
//@@zoom_changes_insert_point_end
...
/*
 * NAME curl_global_init_for_zoom
 * 
 * DESCRIPTION
 *
 * curl_global_init_for_zoom() is speical method for zoom client only, this method will initialize a global variable ${PATH}, 
 * which is used to store OpenSSL key log data.
 *
 * This method should be invoked before curl_global_init(). 
 */
CURL_EXTERN CURLcode curl_global_init_for_zoom(const char *);
 ...
 * DESCRIPTION
 *
 * Returns result of ssl host name vs. CN domain verification.
 */
CURL_EXTERN int Curl_hostcheck(const char* match, size_t matchlen,
